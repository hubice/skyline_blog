        ...
        on(options){//开启
            ...
            layui.use(['form', 'jquery'], function(){
                ...
                form.on('select('+formSelects.options.layFilter+')', function(data){
                    ...
                    var $allChooseId = [];
                    for (var i in formSelects.arr) {
                        formSelects.arr[i] && $allChooseId.push(formSelects.arr[i].val);
                    }
                    $('.'+options.layFilter).val($allChooseId.join(','));
                });

                $('#'+options.id).on('click', 'select[lay-filter="'+formSelects.options.layFilter+'"] + div input', ()=>{
                    formSelects.show();
                });

                $('#'+options.id).on('click', ':not(select[lay-filter="'+formSelects.options.layFilter+'"] + div)', (e)=>{
